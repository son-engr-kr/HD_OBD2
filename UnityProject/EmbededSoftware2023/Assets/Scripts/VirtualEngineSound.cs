...
public class VirtualEngineSound : MonoBehaviour
{
    static VirtualEngineSound _instance;
    [System.Serializable]
    ...
    void Start()
    {
        ...
        _instance.freqChannels[0].Amplitude = 0.7f;
        _instance.freqChannels[1].Amplitude = 0.3f;
        //audioSource.Play();
    }
    public static void EngineSoundWithRPM(float rpm, int cylinderNum)
    {
        //https://patents.google.com/patent/KR20140030426A/ko

        _instance.freqChannels[0].dstFrequency = (rpm * cylinderNum) / (60 * 2);//4는 4기통
        _instance.freqChannels[1].dstFrequency = (rpm * cylinderNum) / (60 * 2) / 2;//4는 4기통
    }
    void Update()
    {
